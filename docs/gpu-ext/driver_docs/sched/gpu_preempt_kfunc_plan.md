...
添加 sleepable kfunc `bpf_nv_gpu_preempt_tsg(hClient, hTsg)`，使 BPF 程序可以从内核上下文（sleepable BPF context，如 bpf_wq callback）直接触发 GPU TSG 抢占，消除 userspace ioctl 往返。
...
// Timeslice 设置（通过 ioctl dispatch 调用）
...
这些函数在 RM 对象系统内部运行，需要有效的 `KernelChannelGroupApi` 对象引用。在 GSP 系统（RTX 5090）上，它们通过 `NV_RM_RPC_CONTROL` 发送 RPC 到 GSP firmware，**会 sleep**。
...
1. **通过 rm_ioctl**：构造完整的 ioctl 参数，调用 `rm_ioctl()`。需要 `nvidia_stack_t`、`nv_state_t`、`nv_file_private_t`，获取这些上下文比较复杂。
2. **新增 RM 导出函数**：在 `kernel_channel_group_api.c` 中添加 `nv_gpu_sched_do_preempt()` 并 `EXPORT_SYMBOL`。更干净，但需要修改 RM 源码。

### 3.3 RM 上下文获取

RM control 调用需要以下上下文：
- `hClient` (NvU32)：RM client handle — 从 BPF tracepoint 捕获
- `hTsg` (NvU32)：TSG object handle — 从 BPF tracepoint 捕获
- `OBJGPU *pGpu`：GPU 对象指针 — 可通过全局函数获取
- `KernelChannelGroupApi *`：通过 `(hClient, hTsg)` 在 RM resource server 中查找

方案 2（新增导出函数）可以在 RM 内部完成所有上下文查找，对 kfunc 暴露最简接口。
...
```c
#include "os-interface.h"  // for rm_ioctl decl

/*
 * nv_gpu_sched_do_preempt - Kernel-internal TSG preempt API
 *
 * Called from BPF kfunc (sleepable context) to preempt a TSG.
 * Bypasses the ioctl path, directly dispatching RM control command.
 *
 * @hClient: RM client handle (captured by BPF tracepoint)
 * @hTsg:    TSG object handle (captured by BPF tracepoint)
 *
 * Returns: NV_STATUS (0 = NV_OK)
 */
NV_STATUS
nv_gpu_sched_do_preempt(NvU32 hClient, NvU32 hTsg)
{
    ...
    // bWait=NV_FALSE for async preempt (don't wait for GPU idle)
...
**注意事项**：
- 使用 `RMAPI_GPU_LOCK_INTERNAL` 获取内部 RM API（不经过 ioctl 安全检查）
- `bWait=NV_FALSE` 使 preempt 异步（不等待 GPU 完全空闲）
- 需要验证 `pRmApi->Control` 是否会自行处理 GPU lock / RPC 序列化
- 可能需要额外的 API security info 设置（参考 escape.c 的 `privLevel = RS_PRIV_LEVEL_KERNEL`）

**备选方案**：如果直接用 `pRmApi->Control` 有权限问题，可改用 `rm_ioctl` 构造完整 ioctl 调用：

```c
NV_STATUS
nv_gpu_sched_do_preempt(NvU32 hClient, NvU32 hTsg)
{
    // 构造 NVOS54_PARAMETERS (NV_ESC_RM_CONTROL 的参数格式)
    NVOS54_PARAMETERS params = { 0 };
    NVA06C_CTRL_PREEMPT_PARAMS preemptParams = { .bWait = NV_FALSE };

    params.hClient = hClient;
    params.hObject = hTsg;
    params.cmd = NVA06C_CTRL_CMD_PREEMPT;  // 0xa06c0105
    params.params = &preemptParams;
...
```c
/* Forward declaration - implemented in nv-kernel.o (RM layer) */
 ...
 * or sleepable struct_ops hook). Triggers GPU TSG preemption via
 * RM internal API, bypassing the userspace ioctl path.
 *
 * @hClient: RM client handle (from BPF tracepoint capture)
 * @hTsg:    TSG object handle (from BPF tracepoint capture)
 *
...
| 文件 | 操作 | 内容 |
|------|------|------|
| `kernel-open/nvidia/nv-gpu-sched-hooks.h` | 修改 | 无需改动（preempt kfunc 不需要新 context struct） |
| `kernel-open/nvidia/nv-gpu-sched-hooks.c` | 修改 | 添加 `bpf_nv_gpu_preempt_tsg` kfunc 定义和注册 |
| `src/nvidia/.../kernel_channel_group_api.c` | 修改 | 添加 `nv_gpu_sched_do_preempt()` + EXPORT_SYMBOL |
| `extension/gpu_sched_preempt.bpf.c` | 新建 | BPF 程序：tracepoint + bpf_wq + preempt kfunc |
...
`nv_gpu_sched_do_preempt()` 绕过 ioctl 安全检查直接调用 RM internal API。需要确保：
- `rmapiGetInterface(RMAPI_GPU_LOCK_INTERNAL)` 返回的 API 不需要额外的 security context
- 如果需要，可能要使用 `RMAPI_API_LOCK_INTERNAL` 或手动设置 `secInfo.privLevel = RS_PRIV_LEVEL_KERNEL`
...
#### 已实现的代码修改（kernel module 侧）

**注意**: 这些修改尚未经过完整的回归验证。

1. **`kernel-open/nvidia/nv-gpu-sched-hooks.h`** + **`src/.../nv-gpu-sched-hooks.h`**
   - `nv_gpu_task_init_ctx` 新增 `hClient`、`hTsg` 字段

2. **`src/.../kernel_channel_group.c`**
   - 移除 `kchangrpInit_IMPL` 中的 hook 调用（此处无 RM handles）

3. **`src/.../kernel_channel_group_api.c`**
   - 在 `kchangrpapiConstruct_IMPL` 中添加 hook 调用（此处有 `pParams->hClient` 和 `pParams->hResource`）

4. **`src/.../escape.c`**
   - 添加 `0xa06c0000` 范围的安全检查绕过（`privLevel = RS_PRIV_LEVEL_KERNEL`）

5. **`src/.../osapi.c`**
   - 新增 `nv_gpu_sched_do_preempt()` 和 `nv_gpu_sched_do_set_timeslice()`
   - 走 `NV_ENTER_RM_RUNTIME` + `threadStateInit` + `Nv04ControlWithSecInfo` 路径

6. **`kernel-open/nvidia/nv-gpu-sched-hooks.c`**
   - 新增 `bpf_nv_gpu_preempt_tsg()` (KF_SLEEPABLE kfunc)
   - 新增 `bpf_nv_gpu_set_timeslice_runtime()` (KF_SLEEPABLE kfunc)

7. **`src/nvidia/exports_link_command.txt`** + **两个 `nv.h`**
   - 新增 `nv_gpu_sched_do_preempt` 和 `nv_gpu_sched_do_set_timeslice` 导出

#### 验证状态
- [x] 模块编译成功
- [x] 模块加载成功，dmesg 确认 struct_ops + kfunc 初始化
- [x] kallsyms 确认 `bpf_nv_gpu_preempt_tsg` 和 `bpf_nv_gpu_set_timeslice_runtime` 已注册
- [x] 现有 struct_ops 程序 (`gpu_sched_set_timeslices`) 在新模块上正常工作
- [ ] **BPF test 程序编译**（vmlinux.h 不含 nvidia module BTF，需重新生成）
...
**限制**：此方案要求 preempt 调用者和 CUDA 在**同一进程**内（共享 fd 表）。跨进程 preempt 仍需要 escape.c bypass 或 kfunc 路径。
...
1. ~~多 TSG 竞争测试~~ ✅ Test E + Test F 完成
2. **跨进程 preempt** — 若需要从 BPF 程序（非同进程）preempt，仍需 kfunc 路径或 escape.c
3. **集成到 BPF 调度策略** — 将 preempt ioctl 集成到 UVM eviction/prefetch 策略中
4. **动态 preempt 策略** — 根据 fault rate / QoS 信号自动决定 preempt 目标和频率

## 9. 参考文档

- [gpu_preempt_ctrl_design.md](./gpu_preempt_ctrl_design.md) — 现有 userspace preempt 工具设计
- [ebpf_preempt_design.md](./ebpf_preempt_design.md) — eBPF preempt 可行性分析
- [GPreempt_Implementation_Analysis.md](./gpreempt-analysis/GPreempt_Implementation_Analysis.md) — GPreempt 论文实现分析
- [hook_enhancement_analysis.md](./hook_enhancement_analysis.md) — GPU 调度 hook 增强分析
...
- `extension/gpu_preempt_ctrl.bpf.c` — 现有 TSG tracepoint BPF 程序
