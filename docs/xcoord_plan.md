...
2. **场景局限**: 只对 LLM serving（少线程、异步）有效。GNN (+3% overhead)、FAISS (+67~759% 灾难性退化) 都不行。论文声称 "跨子系统协调"，但实际只能服务一类 workload。

3. **无主动优化证据**: 唯一证明的效果是 "从 stress-ng 干扰中恢复"。无 stress 时 GNN B5 反而增加 3% overhead。还没有证据表明 xCoord 能**超越** CFS baseline。stress-ng 是人造场景，和生产环境无关。
...
**后续工作必须直面这 5 个问题**: Profiling-first（解决 #4）→ 针对性算法设计（解决 #1）→ 多场景验证（解决 #2）→ 无 stress 主动优化（解决 #3）→ 最小化 overhead（解决 #5）。
