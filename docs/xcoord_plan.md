...
| 问题 | 根因 | 修复 |
|------|------|------|
| `sched_gpu_baseline.bpf.skel.h` not found | Makefile 生成 `*.skel.h` 非 `*.bpf.skel.h` | 改 include 为 `sched_gpu_baseline.skel.h` |
| skeleton struct 名 `sched_gpu_baseline` 未定义 | SCX 宏期望名字不含 `_bpf`，但 skeleton 实际名为 `sched_gpu_baseline_bpf` | 重写 loader 用直接 libbpf API |
...
// sched_gpu_baseline.bpf.c — 新增 worker 查询
...
**关键发现**:
1. **120B UVM batch 模式下, CPU stress 对吞吐量影响有限** (~-1.4%): GPU pipeline 连续运行时可隐藏 CPU scheduling 延迟。注意: 仅在 batch 模式 + gpu_ext 已加载条件下测试, 不能推广到 serving 或无 gpu_ext 场景。
2. **gpu_boosted=1**: worker PID 追踪机制工作，但匹配率极低。可能因 UVM worker 线程生命周期短（处理完 fault 就退出），sched_ext enqueue 时不一定能看到
3. **120B batch 不是 sched_ext 的理想验证场景**: batch 吞吐量对 CPU jitter 不敏感。但 120B **serving** 场景（per-request latency 敏感）仍需评估。
...
Week 2: POC-1 实现 (2026-02-23 ~ 02-24) ✅ 完成
  ├─ ✅ Day 1: shared_maps.h + eviction_lfu_xcoord.bpf.c + loader
  ├─ ✅ Day 1: sched_gpu_baseline.bpf.c + loader（3 轮编译修复）
  ...
  └─ ✅ Day 2: OOM 根因确认 — 残留 llama-server 进程各占 552MiB，清理后推理正常

Week 3: POC-1 验证 + 多 workload 实验 (2026-02-27 ~ 02-28) ✅ 完成
  ├─ ✅ 重写实验脚本 (poc1_xcoord_bench.py)
  ├─ ✅ 3 critical bugs fixed (select_cpu bypass, PID matching, DSQ conflict)
  ├─ ✅ 20B serving: TPOT 完全恢复 (4.44→3.72ms)，详见 §14
  ├─ ✅ W1 GNN: sched_ext overhead +3%，主动优化无效
  ├─ ✅ W2 FAISS: sched_ext 在高 contention 下 +67-759%，不适合
  └─ ✅ 结论: xCoord 适合 LLM serving (少线程、异步)，不适合 batch (多线程、barrier)

Week 4: 性能分析 + 算法设计 ← 当前阶段
  ├─ ⏳ 用 profiling 工具量化 CPU 调度瓶颈（而非盲目跑 baseline）
  ├─ ⏳ 基于 profiling 数据设计针对性算法
  └─ ⏳ 实现 + 评估
```
...
POC-0 结果: ✅ CPU stress 增加 TTFT 6.1x (521ms → 3160ms) → 继续 POC-1
  └─ 且 CPU pinning 无效 (3160ms vs 3170ms) → 强化 xCoord 动机
...
POC-1 完成后实际路径: ✅ TPOT 完全恢复 → blind boost 有效但缺乏 novelty
  ├─ W1 GNN: sched_ext overhead +3%，主动优化无效
  ├─ W2 FAISS: sched_ext 在 batch workload 下有害 (+67-759%)
  └─ → 转向 profiling-first 方法（§15.4），不再盲目跑 baseline

POC-2 (ablation study): ⏳ 需要先完成 profiling，设计针对性算法后再做 ablation
POC-3 (双向协调): ⏳ 待 POC-2 结果决定
```
...
**这一发现也解释了为什么 CPU pinning 对 120B UVM 无效**: `taskset` 只影响用户空间线程，不影响处理 UVM fault 的内核 kworker 线程。xCoord 的 sched_ext 可以直接识别和提升 kworker，是 taskset 不可能做到的。
...
> **原始计划**（2026-02-24）已被 §14 实验结果和 §15 profiling-first 方法论取代。
> - item 1 已完成: 20B 验证成功，详见 §14
> - item 2-3 待做: 但需先 profiling 确认瓶颈再实现，详见 §15.4
> - item 4 仍有效: MSched 作为 related work
...
```
GPU侧 (gpu_ext): track_uvm_worker() → 记录正在处理 fault 的 kworker TID
CPU侧: 只 boost 那个 specific thread (TID 匹配, 非 TGID)
        非 GPU 线程 → SCX_DSQ_LOCAL (零额外开销)
```
- 直接修复 baseline 4 个核心缺陷
- 最小改动可验证 "精准 boost vs 盲目 boost" 区别
- 预期: vLLM/FAISS 场景不再恶化，serving 场景有 recovery
...
|------|--------------------------|---------|
| E1 (120B batch) | CPU ON-CPU 99.7%, overhead 0.33% | 极小 — 但这是 batch，serving 未测 |
| E3 (vLLM 30B) | 无 stress P99 TPOT = 615.6ms vs median = 166.2ms | **P99/median = 3.7x** — CFS 自身调度 jitter 造成的 tail latency |
| E4 (20B serving) | CPU ON-CPU 100% | 极小 — 模型 fit in VRAM，无 UVM |

**无 stress 但有机会的 4 个方向**:

**1. 自然 tail latency reduction（不需 stress-ng）**
- vLLM 30B 无 stress 时 P99 已是 median 的 3.7x → CFS 调度 jitter 本身导致尾延迟
- SLO-sensitive serving 场景中，P99/P999 比 median 重要得多
- 精准 boost GPU critical-path thread 可以减少 CFS 的调度不确定性
- **验证方式**: 无 stress 环境下对比 CFS vs xCoord 的 P99 TPOT
...
**3. kworker 优先级反转（UVM 场景特有）**
- 120B UVM 每 token 约 107 chunks = 107 次 fault handling
- CFS 不区分 GPU-critical kworker vs 普通 kworker (filesystem, RCU, network)
- 即使没有外部 stress，系统自身的 kworker 之间已经在竞争
- 这在 **serving 场景**（持续 fault handling）中比 batch 更明显
- **E5 (120B serving)** 是验证此假设的关键实验

**4. CPU-GPU phase-aware 调度**
- GPU compute 和 CPU DMA 可以 overlap
- CFS 不知道 GPU 当前处于 compute phase（CPU 可降优先级）还是 stall-on-fault phase（CPU 须立即处理）
- gpu_ext 的 fault_rate 信号可以区分这两个 phase
- 不依赖 stress-ng：即使单 workload，phase-aware 调度也能减少 pipeline bubble

**优先级排序（按 novelty + 可行性）**:
1. **多 workload 共存** — novelty 最高（真正的 cross-subsystem coordination），但实验设计复杂
2. **Tail latency reduction** — 有现成 profiling 数据支撑 (P99/median=3.7x)，最容易验证
3. **kworker 优先级反转** — 需要 E5 (120B serving) 的 profiling 数据
4. **Phase-aware 调度** — 实现最复杂，但 novelty 也很高
