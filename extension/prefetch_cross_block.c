/* Cross-VA-Block Prefetch + Passive MRU Eviction - Loader */
#include <stdio.h>
#include <stdlib.h>
#include <signal.h>
#include <unistd.h>
...
#include "prefetch_cross_block.skel.h"
...
int main(int argc, char **argv) {
    struct prefetch_cross_block_bpf *skel;
    ...
    skel = prefetch_cross_block_bpf__open();
    if (!skel) {
        fprintf(stderr, "Failed to open BPF skeleton\n");
        return 1;
    }

    err = prefetch_cross_block_bpf__load(skel);
    if (err) {
        fprintf(stderr, "Failed to load BPF skeleton: %d\n", err);
        goto cleanup;
    }

    link = bpf_map__attach_struct_ops(skel->maps.uvm_ops_cross_block);
    if (!link) {
        err = -errno;
...
cleanup:
    prefetch_cross_block_bpf__destroy(skel);
    return err < 0 ? -err : 0;
}
