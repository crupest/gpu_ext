...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < config.num_streams; i++) {
        ...
        if (config.use_heterogeneous && i < (int)config.kernel_types_per_stream.size()) {
            current_kernel_type = config.kernel_types_per_stream[i];
        }
        ...
        // Only allocate auxiliary buffers that the selected kernel type will use.
        if (current_kernel_type == MEMORY || current_kernel_type == GEMM) {
            d_temp.emplace_back(make_cuda_memory<float>(current_size));
        } else {
            d_temp.emplace_back(nullptr);
        }

        // Allocate result matrix for GEMM
        if (current_kernel_type == GEMM) {
